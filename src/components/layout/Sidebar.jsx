...
const Sidebar = ({ isOpen, onClose, isCollapsed, onToggleCollapse }) => {
    ...
    const navItems = [
        ...
        { id: 'Sales list', icon: FileText, label: 'Sales list' },
        { id: 'Goals', icon: Target, label: 'Goals' },
        { id: 'Dashboard', icon: LayoutGrid, label: 'Dashboard', highlighted: true },
    ];
    ...
    return (
        <>
            ...
            <div className={`
                fixed inset-y-3 left-3 z-30 bg-[#f5f5f5] rounded-3xl transform transition-all duration-300 ease-in-out flex overflow-hidden shadow-sm
            ...
            `}>
                ...
                <div className="w-14 bg-[#f0f0f0] flex flex-col items-center py-4 border-r border-gray-200/50">
                    <div className="w-9 h-9 bg-black rounded-full flex items-center justify-center text-white font-bold text-sm mb-6
                    ...
                    <div className="flex-1 flex flex-col items-center space-y-1">
                        {navItems.map((item) => (
                            <IconButton
                                ...
                                highlighted={item.highlighted && activeItem === item.id}
                    ...
                    <div className="flex flex-col items-center space-y-2 mt-auto">
                        <button className="w-9 h-9 rounded-xl flex items-center justify-center text-gray-400
                                         hover:bg-gray-200 hover:text-gray-600 active:scale-90 transition-all duration-200">
                            <HelpCircle size={18} />
                        </button>
                        <button className="w-9 h-9 rounded-xl flex items-center justify-center text-gray-400
                                         hover:bg-gray-200 hover:text-gray-600 active:scale-90 transition-all duration-200">
                ...
                <div className="w-48 flex flex-col bg-[#f5f5f5]">
                    <div className="h-14 flex items-center px-3 border-b border-gray-200/30">
                        ...
                        <button
                            onClick={onClose}
                            className="lg:hidden text-gray-500 p-1 rounded-full hover:bg-gray-200 hover:text-gray-900 active:scale-90 transition-all duration-200"
                        >
                            <X size={16} />
                        </button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto py-3 px-2">
                        <div className="px-2 mb-3">
                            <div className="flex items-center justify-between">
                                ...
                                {activeItem === 'Dashboard' && (
                                    <Plus size={16} className="text-gray-400 hover:text-gray-600 cursor-pointer transition-colors" />
                                )}
                            </div>
                        </div>
                        ...
                        <div className="mt-4 px-2 py-2 text-sm text-gray-500 flex items-center cursor-pointer hover:text-gray-700 transition-colors duration-200 group rounded-lg hover:bg-gray-200/50">
                            <Settings size={14} className="mr-2 group-hover:rotate-45 transition-transform duration-300" />
                            Manage folders
                        </div>
                    </div>
                </div>
...
const IconButton = ({ icon: Icon, active, highlighted, onClick, label }) => (
    <button
        className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200 active:scale-90
            ${highlighted ? 'bg-primary/15 text-primary' : active ? 'bg-white text-gray-700 shadow-sm' : 'text-gray-400 hover:bg-gray-200 hover:text-gray-600'}
        `}
        onClick={onClick}
        title={label}
    >
...
const FolderItem = ({ label, expanded, onToggle, children }) => (
    <div className="mb-1">
        <div
            className="flex items-center px-2 py-2 text-sm font-medium text-gray-600 cursor-pointer hover:text-gray-900 hover:bg-gray-200/50 rounded-lg group transition-all duration-200 active:scale-[0.98]"
            onClick={onToggle}
        >
...
const SubItem = ({ label, badge, badgeColor = "bg-primary text-white", activeText, active, onClick }) => (
    <div
        className={`flex items-center justify-between px-2 py-1.5 text-[13px] cursor-pointer rounded-md transition-all duration-200
            ${active ? 'bg-gray-200/70' : 'hover:bg-gray-200/50'}
            ${activeText ? 'text-primary font-medium' : 'text-gray-600 hover:text-gray-900'}
            active:scale-[0.98]
        `}
        onClick={onClick}
    >
        <span className="truncate">{label}</span>
        {badge && (
            <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium flex-shrink-0 ml-1 ${badgeColor}`}>
                {badge}
            </span>
        )}
    </div>
