    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6">
            <div className="lg:col-span-2">
                ...
                <div className="mt-2 text-xs sm:text-sm text-gray-500 font-medium">
                    vs prev. <span className="text-gray-700">$501,641.73</span> Jun 1 - Aug 31, 2023
                </div>
            </div>

            <div className="lg:col-span-2 flex gap-3 sm:gap-4 overflow-x-auto pb-2 -mx-6 px-6 sm:-mx-4 sm:px-4 lg:mx-0 lg:px-0">
                <div className="flex-1 min-w-[140px] sm:min-w-[180px] bg-white p-3 sm:p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between
                    ...
                    <div className="flex items-center justify-between mt-3 sm:mt-4">
                        <div className="flex items-center text-xs sm:text-sm font-medium text-gray-700">
                            <img src="https://i.pravatar.cc/150?u=mikasa" className="w-4 h-4 sm:w-5 sm:h-5 rounded-full mr-2" alt="" />
                            Mikasa
                        </div>
                ...
                <div className="flex-1 min-w-[140px] sm:min-w-[180px] bg-black p-3 sm:p-4 rounded-2xl shadow-sm flex flex-col justify-between text-white relative overflow-hidden
                              hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 cursor-pointer group">
                ...
                <SmallStatCard label="Deals" value="256" subValue="5" type="down" active={activeCard === 'Deals'} onClick={() => setActiveCard('Deals')} />
                <SmallStatCard label="Value" value="528k" subValue="7.9%" type="up" active={activeCard === 'Value'} onClick={() => setActiveCard('Value')} />
                <SmallStatCard label="Win rate" value="44%" subValue="1.2%" type="up" active={activeCard === 'Win rate'} onClick={() => setActiveCard('Win rate')} />
            </div>
        </div>
    )
}

const SmallStatCard = ({ label, value, subValue, type, active, onClick }) => (
    <div
        className={`min-w-[70px] sm:min-w-[90px] p-2 sm:p-3 rounded-2xl border flex flex-col items-center justify-center text-center cursor-pointer
            transition-all duration-200 hover:scale-105 active:scale-95
            ${active ? 'bg-white border-primary shadow-[0_0_0_1px_rgba(225,29,72,1)]' : 'bg-white border-gray-100 hover:border-gray-200 hover:shadow-md'}
        `}
        onClick={onClick}
    >
